...
class EventEntry:
    ...
    relative_date: str | None = None  # "yesterday", "last week", etc.
    resolved_date: str | None = None  # Calculated actual date
...
class EventDateIndex:
    ...
    EVENT_PATTERNS = [
        ...
        (
            r'\b(?:watched|watch|watching)\s+["\']?([^"\']+?)["\']?(?:\s+for\s+the\s+first\s+time)?',
            "watched",
        ),
        ...
        (
            r"\b(?:started|began|begin)\s+(?:playing|learning|taking)\s+(?:the\s+)?(\w+)",
            "started_activity",
        ),
        ...
        (
            r"\b(?:got|adopted|rescued)\s+(?:a\s+|my\s+|our\s+)?(?:dog|puppy|pet)\s+(?:named\s+)?(\w+)?",
            "got_pet",
        ),
        (r"\b(?:dog|puppy)\s+(?:named\s+)?(\w+)", "got_pet"),
        # Activities - expanded for LoCoMo events
        (
            r"\b(went|go|going)\s+(?:to\s+)?"
            r"(?:the\s+|a\s+|an\s+)?"
            r"(camping|hiking|biking|swimming|running"
            r"|park|museum|concert|parade|pride parade"
            r"|workshop|conference|meeting|class|fair"
            r"|support group|lgbtq\+? support group"
            r"|activist group|mentorship program"
            r"|picnic)",
            "activity",
        ),
        (
            r"\b(went|go|going)\s+(?:to\s+)?"
            r"(?:the\s+|a\s+|an\s+)?lgbtq\+?\s+"
            r"(pride parade|parade|support group"
            r"|conference|meeting|event)",
            "lgbtq_activity",
        ),
        (
            r"\b(attended|attend|attending)\s+"
            r"(?:a\s+|the\s+|an\s+)?"
            r"(workshop|conference|meeting|parade"
            r"|pride parade|class|event|session|group"
            r"|support group|lgbtq\+?|mentorship"
            r"|program)",
            "attended",
        ),
        (
            r"\b(signed up|sign up|registered|joined)"
            r"\s+(?:for\s+)?(?:a\s+|the\s+|an\s+)?"
            r"(?:new\s+)?"
            r"(\w+(?:\s+\w+)?(?:\s+\w+)?(?:\s+\w+)?)",
            "signed_up",
        ),
        ...
        (
            r"\b(took|take|taking)\s+(?:my\s+|the\s+)?(?:fam(?:ily)?|kids|children)?\s*(?:to\s+(?:a\s+|the\s+)?)?(camping|hiking|swimming|park)",
            "family_activity",
        ),
        ...
        # Creative activities
        (
            r"\b(painted|drew|draw|drawing)\s+(?:a\s+)?(?:the\s+)?(?:my\s+)?(\w+(?:-\w+)?(?:\s+\w+)?)",
            "created",
        ),
        (
            r"\b(?:self-portrait|selfportrait)\s+(?:I|i|we)\s+(?:made|created|drew|painted)",
            "self_portrait",
        ),
        ...
        (
            r"\b(bought|got|purchased|received)\s+(?:a\s+|the\s+|some\s+)?(\w+(?:\s+\w+)?)",
            "acquired",
        ),
        # Business/work events
        (r"\b(opened|started|launched|began)\s+(?:a\s+|the\s+|my\s+)?(\w+(?:\s+\w+)?)", "started"),
        (
            r"\b(got accepted|was accepted|accepted)\s+(?:for\s+|to\s+)?(?:a\s+|the\s+)?(\w+)",
            "accepted",
        ),
        ...
        (
            r"\b(?:passed|completed|finished)\s+(?:the\s+)?(?:\w+\s+)*?(?:interview|interviews|test|exam)",
            "passed_interview",
        ),
        # Life events
        (r"\b(adopted|married|divorced|moved|graduated)", "life_event"),
        (
            r"\b(friend|brother|sister|husband|wife)\s+(\w+)\s+(?:adopted|married|got|had)",
            "friend_event",
        ),
    ...
    def add_from_message(
        self, speaker: str, content: str, session_date: str, session_idx: int = 0
    ) -> list[EventEntry]:
        """
        ...
        other_entity_patterns = [
            ...
            (
                r"(\w+)\s+(?:said|told me|mentioned)\s+(?:that\s+)?(?:she|he)\s+(.+?)(?:\.|$)",
                "friend_said",
            ),
        ]
    ...
    def answer_temporal_question(self, question: str, entity: str | None = None) -> str | None:
        """
        ...
        for phrase, event_type in specific_event_mapping.items():
            if phrase in q_lower:
                entries = [
                    e
                    for e in self.by_event_type.get(event_type, [])
                ...
                if entries:
                    ...
                    if len(entries) > 1:
                        ...
                        item_keywords = [
                            ...
                            "vase",
                            "mug",
                        ]  # Removed 'pot' - conflicts with 'pottery'
                        ...
                        if "make" in q_lower or "made" in q_lower:
                            for e in entries:
                                ...
                                if (
                                    ...
                                    or "making" in text_lower
                                ):
                                    # Prefer "made it" pattern
                                    # (actually made vs signed up)
